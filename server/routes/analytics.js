...
router.get('/dashboard', protect, authorize('admin'), async (req, res) => {
  try {
    ...
    const cancelledCount = await Parcel.countDocuments({ status: 'Cancelled' });
...
// @access  Private (Admin only)
router.get('/users', protect, authorize('admin'), validate([
  query('role').optional().isIn(['customer', 'shopkeeper']).withMessage('Role must be customer or shopkeeper'),
]), async (req, res) => {
...
// @access  Private (Admin only)
router.delete('/users/:id', protect, authorize('admin'), validate([
