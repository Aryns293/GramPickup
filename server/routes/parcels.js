...
const { addTimelineEvent } = require('../utils/parcelTimeline');
const { getFeeDetails } = require('../utils/fees');

const objectIdParam = [
  param('id').isMongoId().withMessage('Valid parcel id is required'),
];

const createParcelRules = [
  body('parcelName').trim().isLength({ min: 2, max: 120 }).withMessage('Parcel name must be 2 to 120 characters'),
  body('trackingNumber').trim().isLength({ min: 4, max: 60 }).matches(/^[A-Za-z0-9\-]+$/).withMessage('Tracking number must be alphanumeric'),
  body('shopId').isMongoId().withMessage('Valid shop id is required'),
  body('expectedArrivalDate').isISO8601().toDate().withMessage('Valid expected arrival date is required'),
];
...
const cancelRules = [
  ...objectIdParam,
  body('reason').optional({ checkFalsy: true }).trim().isLength({ max: 250 }).withMessage('Reason must be 250 characters or less'),
];

// @desc    Register expected parcel
// @route   POST /api/parcels
// @access  Private (Customer only)
router.post('/', protect, authorize('customer'), validate(createParcelRules), async (req, res) => {
  ...
  try {
    ...
    const parcel = await Parcel.create({
      ...
      timeline: [{
        status: 'Expected',
        note: 'Parcel registered by customer',
        actorRole: 'customer',
      }],
    });
...
// @access  Private (Shopkeeper only)
router.put('/:id/received', protect, authorize('shopkeeper'), validate(objectIdParam), async (req, res) => {
  try {
    ...
    addTimelineEvent(parcel, 'Arrived', 'Parcel marked as arrived at pickup shop', 'shopkeeper');
...
// @access  Private (Shopkeeper only)
router.put('/:id/ready', protect, authorize('shopkeeper'), validate(objectIdParam), async (req, res) => {
  try {
    const parcel = await Parcel.findById(req.params.id);
    if (!parcel) {
      return res.status(404).json({ message: 'Parcel not found' });
    ...
    addTimelineEvent(parcel, 'Ready for Pickup', 'OTP generated and parcel marked ready for pickup', 'shopkeeper');
    ...
    await Notification.create({
      ...
      message: `Your parcel "${parcel.parcelName}" (Tracking: ${parcel.trackingNumber}) is ready for pickup at "${shop.shopName}". Visit the shop with your tracking number to collect. Current fee: ₹${details.fee}.`,
    });
...
// @access  Private (Shopkeeper only)
router.put('/:id/deliver', protect, authorize('shopkeeper'), validate(otpRules), async (req, res) => {
  const { otp } = req.body;
  ...
  try {
    ...
    addTimelineEvent(parcel, 'Delivered', 'Parcel delivered after OTP verification', 'shopkeeper');
...
// @access  Private (Customer only)
router.put('/:id/cancel', protect, authorize('customer'), validate(cancelRules), async (req, res) => {
  ...
  try {
    ...
    if (parcel.status !== 'Expected') {
      return res.status(400).json({ message: 'Only expected parcels can be cancelled' });
    }

    parcel.status = 'Cancelled';
    parcel.cancellationReason = reason || 'Cancelled by customer before arrival';
    addTimelineEvent(parcel, 'Cancelled', parcel.cancellationReason, 'customer');
